...
class Game {
  constructor() {
    this.places = PLACES
    this.peopleInPlaces = PEOPLE_IN_PLACES
    this.users = new Map()
    this.id = v4()
    this.status = {
      name: 'waiting-room',
  ...
  changeConfig({ timerInS, howMuchSpys, rodadas }) {
    this.config.timerInS = timerInS ?? this.config.timerInS
    this.config.howMuchSpys = howMuchSpys ?? this.config.howMuchSpys
    this.config.rodadas = rodadas ?? this.config.rodadas
...
io.on('connection', (socket) => {
  socket.on('create-room', ({ name }) => {
    ...
    newGame.join({ id: newSessionId, name })
    cache.set(newGame.getId(), newGame)

    socket.join(newGame.getId())
    socket.emit('joined-room', {
      sessionId: newSessionId,
      roomId: newGame.getId(),
    })
  ...
  socket.on('join-room', ({ name, roomId }) => {
    ...
    game.join({ id: newSessionId, name })
    ...
    io.to(game.getId()).emit('users', { users: game.getUsers() })

    socket.data = { sessionId: newSessionId, roomId: game.getId(), name }
  })
